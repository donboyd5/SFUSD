...
```{r locations}
dsacs <- r"(E:\data\California\CA_sacs/)"
dsacs_all <- paste0(dsacs, "data/allyears/")

dj90 <- r"(E:\data\California\CA_j90/)"
...
tabdata2 <- topn %>%
  bind_rows(bay25 %>% 
              ...
              mutate(dname="25 Bay Area districts with largest OPEB expenditures per pupil"),
            nonzero %>% 
              summarise(n=n(),
                        opebpp=median(opebpp),
                        k12ada=median(k12ada)) %>%
...
tab <- tabdata %>%
  ...
  cols_label(
    ...
    k12ada=html("Average daily attendance"),
    opebpp=html("Retiree health care<br>expenditures per pupil")
  ) %>%
  tab_spanner("District value or group median",
                columns=c(k12ada, opebpp)) %>%
...
## REPORT: Bar chart of Bay Area opebpp
```{r bay_hbars}

# pick one or the other of the two combinations below
fnbase <- "bayarea_hbars"; clrs <- c("darkgreen", "blue")
fnbase <- "bayarea_hbars_green"; clrs <- c("darkgreen", "darkgreen")


# get plot data
pdata <- baytop %>%
  filter(k12ada_rank <= topn) %>%
  select(ccode, dcode, codist, k12ada_rank, opebpp_rank) %>%
  left_join(opebdf %>% filter(year==2020), by=c("ccode", "dcode")) %>%
  mutate(codist=factor(codist, levels=baytop %>% arrange(opebpp_rank) %>% .$codist)) %>%
  select(year, ccode, dcode, codist, dname, sfusd, k12ada_rank, opebpp_rank, opebpp) %>%
  mutate(opebpp=ifelse(opebpp==0, 1.5, opebpp))

p <- pdata %>% 
  ggplot(aes(y=reorder(dname, opebpp), x=opebpp, fill=sfusd)) +
  geom_col(width = 0.7) +
  scale_y_discrete(name=NULL) +
  scale_x_continuous(name="Amount per pupil", 
  ...
  ggtitle("OPEB expenditures per pupil in 2020 for 25 largest Bay Area unified school districts") +
  labs(caption=paste0("Source: ", sacs_source)) +
  legend_none +
  caption_left
...
ggsave(plot=p, filename=here::here("results", paste0(fnbase, ".png")), width=10, height=6.5, units="in")
write_csv(pdata, here::here("results", paste0(fnbase, "_data.csv")))
...
## SACS SEPARATE BAY AREA AND TOTAL BAR CHART
```{r 3_bars, include=TRUE}

pbase <- opebdf %>% 
  filter(year==2020, eval(topada)) %>%
  select(year, ccode, dcode, dname, sfusd, bayarea, opebpp)

pall <- pbase %>%
  summarise(value=median(opebpp)) %>%
  mutate(name="all")
...
pdata <- bind_rows(pall, pbay, psfusd) %>%
  mutate(namef=factor(name,
                      levels=c("all", "bay", "SFUSD"),
                      labels=str_wrap(
                        c("All 531 districts with at least 1,000 pupils (median)",
                          "25 Bay Area districts with largest per-pupil OPEB expenditures (median)",
                          "SFUSD"),
                        30)))

p <- pdata %>% 
  ggplot(aes(x=reorder(namef, desc(value)), y=value)) +
  ...
  theme_bw() +
  theme(axis.text.y = element_text(hjust = 0)) +
  ggtitle("General Fund OPEB expenditures per pupil in 2020") +
  labs(caption=paste0("Source: ", sacs_source)) +
  legend_none +
  caption_left
# +  annotate(geom="text", x=mdn + .1, y=5, label="median", hjust=0) 
p
ggsave(plot=p, filename=here::here("results", "3values_vbars.png"), width=10, height=6.5, units="in")
write_csv(pdata, here::here("results", "3values_vbars_data.csv"))
...
```{r set_j90data_year}
#.. set_j90data_year ----
j90data_year <- 2020  # rereun all 4-panel graph chunks and associated tables when this is changed
...
opeb_dists <- opeb_j90 %>%
  filter(plantype=="health", year==j90data_year, eval(topada), opebfte > 0, agecat %in% c("under66", "age66p")) %>%
...
title_eecost <- paste0("Retiree portion of premium in ", j90data_year, ", averaged across district plans")
title_ercost <- paste0("School district portion of premium in ", j90data_year, ", averaged across district plans")
title_cost <- paste0("Total premium (district plus retiree portions) in ", j90data_year, ", averaged across district plans")

params <- tribble(
  ~var, ~title, ~fnbase,
  "toteecost", title_eecost, "retshare",
  "totercost", title_ercost, "sdshare",
  "totcost", title_cost, "totshare"
  )
...
```{r 4panel_allopeb, include=TRUE, fig.width=12, fig.height=8}
...
# subt <- str_wrap(subt)
suffix <- paste0("_allopeb_", j90data_year, ".png")
...
pf <- function(var){
  ...
  theme_bw() +
  caption_left
  ggsave(plot=p, filename=fname, width=10, height=8, units="in")
  p
}
...
title <- paste0("District-reported number of OPEB participants (FTE) in ", j90data_year, ", shown per 1,000 pupils (calculated)")
...
ggsave(plot=p_fte_mean, filename=fname, width=10, height=8, units="in", scale=1.0)
...
```{r 4panel_categoryopeb, include=TRUE, fig.width=12, fig.height=8}
subt <- "Certificated retirees, by age group. Includes districts, with at least 1,000 pupils, that provide\nhealth premium subsidies in the given age category."
# subt <- str_wrap(subt)
suffix <- paste0("_categoryopeb_", j90data_year, ".png")
...
tab <- tabdata %>%
  ...
  gt() %>%
  tab_header(
      title = paste0("Retiree health plan participants and premium costs in ", j90data_year),
...
fnbase <- paste0("table_j90_sfusd_vs_means_allopeb_", j90data_year)
gtsave(tab, here::here("results", paste0(fnbase, ".png")))  # , zoom = 1.5, expand = 10)
write_csv(tabdata, here::here("results", paste0(fnbase, "_data.csv")))
...
fnbase <- paste0("table_j90_sfusd_vs_means_categoryopeb_", j90data_year)
gtsave(tab, here::here("results", paste0(fnbase, ".png")))  # , zoom = 1.5, expand = 10)
write_csv(tabdata, here::here("results", paste0(fnbase, "_data.csv")))
...
tab <- tabdata %>%
  ...
  tab_header(
      title = "SFUSD health plans for certificated retirees in 2020",
