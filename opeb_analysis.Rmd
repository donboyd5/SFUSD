...
We don't bother with this because the data show no spending on actives in any year by SFUSD.

This is a bit mysterious as we might have thought the $25 million payment in FY 2020 would be treated as prefunding for actives. But the relevant SACS codes are all zero.


#### Largest districts (by attendance)
...
### California large districts' general fund OPEB spending over time
...
bay_unified <- 
  # get bay area unifieds
  leas %>%
  filter(year==2020, ccode %in% baycodes, dtypef=="unified") %>%
  select(ccode, dcode, dname) %>%
  # get county name
  left_join(counties %>% select(ccode, coname), by=c("ccode")) %>%
  # get opebpp
  left_join(opeb2 %>%
              filter(year==2020, ccode %in% baycodes) %>%
              select(year, ccode, dcode, k12ada, opebpp),
            by=c("ccode", "dcode")) %>%
  # sort by size then keep top n
  arrange(desc(k12ada)) %>%
  filter(row_number() <= n) %>%
  # sort by opebpp in 2020 to use for plot order
  arrange(desc(opebpp)) %>%
  mutate(codist=paste0(coname, ": ", dname),
         sort=row_number()) %>%
  select(year, ccode, dcode, dname, codist, k12ada, opebpp, sort)



# get plot data
pdata <- bay_unified %>%
  select(ccode, dcode, codist) %>%  # use the 2020 name as they can change over time
  left_join(opeb2 %>%
              select(year, ccode, dcode, activespp, retiredpp, opebpp),
            by = c("ccode", "dcode")) %>%
  pivot_longer(cols=(ends_with("pp"))) %>%
  mutate(codist2=factor(codist, levels=bay_unified$codist))
...
p <- pdata %>%
  ...
  geom_point(colour="blue") +
  theme_bw() +
  facet_wrap(~codist2, ncol=5, labeller=label_wrap_gen(width = 37, multi_line = TRUE)) +
  ggtitle(label="General Fund OPEB expenditures per pupil over time. CAUTION: Nominal.",
          subtitle="25 largest Bay Area school districts, by attendance. Districts ordered by 2020 per-pupil OPEB spending.") +
  scale_y_continuous(name="$ per pupil", labels=comma) +
  scale_x_continuous(name=NULL) +
  labs(caption="NOTE: Includes payments for retirees plus any trust contributions for actives") +
  caption_left
...
check <- opeb_j90 %>%
  filter(plantype=="health", year==2019, 
         # ccode %in% baycodes, 
