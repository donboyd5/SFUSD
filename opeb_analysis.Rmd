...
pdata <- opeb_j90 %>%
  ...
  mutate(erpct=totercost / sum(totercost),
         ...
         agecat=factor(agecat, levels=c("under66", "age66p"), labels=c("Under 65", "Age 65+")))
...
p <- pdata %>%
  mutate(lbl=lbl) %>%
  ggplot(aes(x=planeecost, y=opebfte, size=erpct, colour=agecat, label=lbl)) +
  geom_point() +
...
tabdata1 <- opeb_j90 %>%
  ...
  mutate(erpct=totercost / sum(totercost),
         coverage=as.character(coverage),
         coverage=ifelse(coverage=="twoparty", "two party", coverage)) %>%
  select(year, ccode, dcode, dname, provider, coverage, agecat, erpct, opebfte, 
...
other <- tabdata1 %>%
  filter(!topn) %>%
  group_by(year, ccode, dcode, dname, agecat, topn) %>%
...
tabdata2 <- bind_rows(tabdata1 %>% 
                     ...
                     other %>% 
                       mutate(group=2)) %>%
  arrange(desc(agecat), group, desc(erpct)) %>%
  mutate(plan=ifelse(topn, paste0(provider, ": ", coverage), "All other"),
         agecat=as.character(agecat)) %>%
  select(group, agecat, plan, erpct, opebfte, plancost, planeecost, planercost)
...
totals <- tabdata2 %>%
  summarise(across(contains("cost"), ~ sum(.x * opebfte) / sum(opebfte)),
            across(c(erpct, opebfte), sum),
            .groups="drop") %>%
  mutate(group=4, agecat="total")

tabdata <- bind_rows(tabdata2, subtotals %>% mutate(plan="  Subtotal"), totals %>% mutate(plan="  Grand total")) %>%
  mutate(agecat=factor(agecat,
                       levels=c("age66p", "under66", "total"),
                       labels=c("Age 65+", "Under 65", ""))) %>%
  arrange(agecat, group, desc(erpct))
...
# District cost per participant
```{r}
pdata <- opeb_j90 %>%
  filter(plantype=="health", year==2020, ada >= 1000, opebfte > 0) %>%
  group_by(year, ccode, dcode, dname, agecat) %>%
...
sfusd_points <- pdata %>% 
  filter(eval(sfusd)) %>% 
  select(ccode, dcode, dname, agecat, name, value) %>%
  mutate(label="SFUSD")

mdns <- pdata %>% 
  group_by(agecat, name) %>% 
  summarise(mdn=median(value), .groups="drop")
...
pf <- function(var){
  ...
  ggtitle(title,
          subtitle="Certificated retirees, by age group") +
  labs(caption=capt) +
  theme_bw() +
  caption_left
...
p <- p_tot + p_er + p_ee + plot_layout(ncol = 2)
