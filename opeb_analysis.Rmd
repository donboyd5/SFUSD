...
# show top 10, median of 25-largest bay area districts, median overall
glimpse(opebdf)
tabdata1 <- opebdf %>%
  filter(year==2020, eval(topada)) %>%  # no negatives , opebpp >= 0
  mutate(opebpp=ifelse(opebpp >= 0, opebpp, 0)) %>%
...
## REPORT: Opeb per pupil vs. attendance, log scale
...
pdata1 <- opebdf %>%
  filter(year==2020, eval(topada), opebpp >= 0) %>%  # drop negative opebpp
...
## REPORT: Bay Area large districts' general fund OPEB spending over time
...
baytop <- opebdf %>%
  filter(year==2020, bayarea, dtypef=="unified", opebpp >= 0, eval(topada)) %>%
...
tabdata <- opeb_j90 %>%
  ...
  group_by(year, ccode, dcode) %>%  # don't group by dname as it may change over time
  ...
  summarise(ndists=n(),
            ...
            .groups="drop") %>%
  mutate(pctof_hasopeb=nhealth66pfte / nopebfte,
         pct_ofdists=nhealth66pfte / ndists)
...
sf <- opeb_j90 %>%
  ...
  filter(row_number()==1) %>%
  ungroup %>%
  mutate(ben65p=ifelse(ben65p, "yes", "no"))
  count
...
pdata <- opeb_j90 %>%
  ...
  mutate(erpct=totercost / sum(totercost),
         ...
         lbl=costshare,
         lbl1=paste0(costshare, " share of SFUSD OPEB cost"),
         lbl2=paste0(lbl, ".  SFUSD: ", ercost),
         lbl3=paste0(lbl2, "\n", plan),
         lbl4=paste0("Share of total: ", costshare,
                     ".  SFUSD payment: ", ercost,
                     " Plan: ", plan),
         lbl5=paste0("% of total: ", costshare, ".  SFUSD: ", ercost, "\n", plan),
         lbl=ifelse(erpct > .05, lbl, ""),
         lbl1=ifelse(erpct > .05, lbl1, ""),
         lbl2=ifelse(erpct > .05, lbl2, ""),
         lbl3=ifelse(erpct > .05, lbl3, ""),
         lbl4=ifelse(erpct > .05, lbl4, ""),
         lbl5=ifelse(erpct > .05, lbl5, ""))
...
#   geom_point() +
#   # geom_text(nudge_x = 300, size=3, hjust=0, ) +
#   geom_text_repel(nudge_x = 350, size=2.5, hjust=0, ) +
#   # geom_text(position = position_jitter(width = 5, height = 5), size=3, hjust=0, ) +
#   # geom_text(nudge_x = 650, size=2, hjust=0, lineheight = .75) +
#   scale_y_continuous("# of retirees in the plan", labels = scales::label_comma()) +
#   scale_x_continuous("Annual cost to the plan member", labels=scales::label_comma(prefix="$")) +
#   scale_size_continuous(guide = "none") +
#   scale_colour_manual(values=c("darkgreen", "blue")) +
#   ggtitle("SFUSD Retiree health plans for certificated staff in 2020",
#           "Size of dot and number represent share of employer OPEB costs for certificated retirees. Dollar amount is district cost.") +
#   labs(caption=capt) +
#   theme_bw() +
#   caption_left +
#   legend_notitle
...
tabdata <- bind_rows(tabdata2, 
  ...
  mutate(agecatf=factor(agecat,
                       levels=c("age66p", "under66", "total"),
                       labels=c("Age 65+", "Under 65", ""))) %>%
...
## J90 districts providing benefits beyond age 65
...
base <- opeb_j90 %>%
  ...
  summarise(k12ada=first(k12ada),
            across(c(opebfte, starts_with("tot")), sum), .groups="drop") %>%
  pivot_wider(names_from = agecat,
...
```{r mean_version, include=TRUE}

pdata <- opeb_j90 %>%
  filter(plantype=="health", year==2020, eval(topada), opebfte > 0) %>%
  group_by(year, ccode, dcode, dname, agecat, agecatf) %>%
  summarise(across(c(opebfte, starts_with("tot")), sum), .groups="drop",
            k12ada=first(k12ada)) %>%
  mutate(across(c(totcost, totercost, toteecost), ~ .x / opebfte)) %>%  # average costs
...
pf <- function(var){
  ...
  fname <- here::here("results", paste0("plancost_", fnbase, ".png"))
  ...
  ggtitle(title,
          subtitle="Certificated retirees, by age group. Only includes districts that provide subsidies and that have at least 1,000 pupils.") +
  labs(caption=capt) +
  theme_bw() +
  caption_left
...
fname <- here::here("results", paste0("fteada_means.png"))
...
p
ggsave(plot=p, filename=here::here("results", "combined_means.png"), width=12, height=8, units="in", scale=1.5)
...
### Version that limits age 65+ statistics to districts offering this coverage
```{r plan_costs_ftes_means, include=TRUE}
distbase <- opeb_j90 %>%
  filter(plantype=="health", year==2020, eval(topada), opebfte > 0) %>%
  group_by(year, ccode, dcode, dname, agecat, agecatf) %>%
  summarise(across(c(opebfte, starts_with("tot")), sum), .groups="drop",
...
tab <- tabdata %>%
  ...
  tab_header(
      ...
      subtitle = "Younger retirees and older retirees"
    ) %>%
  cols_label(
    name="",
    mean=html("Mean<br>across<br>districts"),
    pctdiff=html("SFUSD<br>% above<br>or below<br>mean")
  ) %>%
...
gtsave(tab, here::here("results", "j90_sfusd_vs_means.png"))  # , zoom = 1.5, expand = 10)

```



### Version that includes non-offering districts in the age 65+ statistics
```{r plan_costs_ftes_means_v2, include=TRUE}
...
opeb_offered <- expand_grid(opeb_dists, 
                            agecats)
count(opeb_offered, agecat, agecatf)

opeb_base <- opeb_offered %>%
  left_join(opeb_j90 %>%
              select(year, ccode, dcode, dname, plantype, agecat, opebfte, starts_with("tot")),
            by = c("year", "ccode", "dcode", "dname", "plantype", "agecat")) %>%
  select(year, ccode, dcode, dname, plantype, agecat, agecatf, opebfte, starts_with("tot"))

opeb_base %>% filter(eval(sfusd))
  


opeb_base2 <- opeb_base %>%
  mutate(across(c(opebfte, starts_with("tot")), ~ replace_na(.x, 0)),  # set values to zero if we did not match
         across(c(opebfte, starts_with("tot")), ~ ifelse(.x < 0, 0, .x))) %>% # 2 oddball negatives
  group_by(year, ccode, dcode, dname, agecat, agecatf) %>%
  summarise(across(c(opebfte, starts_with("tot")), sum), .groups="drop",
            k12ada=first(k12ada)) %>%
  mutate(across(c(totcost, totercost, toteecost), ~ .x / opebfte),  # average costs
         kfteada=opebfte / (k12ada / 1000)) 
distbase %>% filter(eval(sfusd))
summary(distbase)
count(distbase, agecat)
...
gtsave(tab, here::here("results", "j90_sfusd_vs_means_v2.png"))  # , zoom = 1.5, expand = 10)
...
# District cost per participant for districts with opebfte
```{r mdn_version, include=TRUE}
...
mdns <- pdata %>% 
  group_by(agecat, agecatf, name) %>% 
  summarise(n=n(), mdn=median(value), .groups="drop")
...
pf <- function(var){
  ...
  boostup <- 22
  x1 <- 1.3
  y1 <- mdns %>% filter(name==var, agecat=="under66") %>% .$mdn + boostup
  
  x2 <- 2.3
  y2 <- mdns %>% filter(name==var, agecat=="age66p") %>% .$mdn + boostup
  ...
  annotate("text", 
           x=x1, 
           y=y1,
           size=2.5, label="median", colour="darkgreen") +
  annotate("text", 
           x=x2, 
           y=y2,
           size=2.5, label="median", colour="darkgreen") +
...
mdnsnew <- pdatanew %>% 
  group_by(agecat, agecatf) %>% 
  summarise(n=n(), mdn=median(kfteada), .groups="drop")

title <- "District-reported number of OPEB participants (FTE), shown per 1,000 pupils (calculated)"
fname <- here::here("results", paste0("fteada.png"))
  
boostup <- 0.5
x1 <- 1.3
y1 <- mdnsnew %>% filter(agecat=="under66") %>% .$mdn + boostup
  
x2 <- 2.3
y2 <- mdnsnew %>% filter(agecat=="age66p") %>% .$mdn + boostup
...
ggsave(plot=p_fte, filename=fname, width=8, height=8, units="in")


#.. combine plots ----
p <- p_fte + p_tot + p_er + p_ee + plot_layout(ncol = 2)
p
ggsave(plot=p, filename=here::here("results", "combined.png"), width=12, height=8, units="in", scale=1.5)
