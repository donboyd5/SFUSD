...
ggsave(plot=p_fte_mean, filename=fname, width=8, height=8, units="in", scale=1.25)
...
### REPORT BODY TABLE WITH DAVID CRANE's SORT: Final summary table including all OPEB districts in the age 65+ statistics

```{r plan_costs_ftes_means_allopeb_cranesort, include=TRUE}
# sorted by old group first
# this version includes all districts regardless of whether they provide benefits in an age category
distcosts <- opeb_base %>%
  select(year, ccode, dcode, dname, agecat, agecatf, starts_with("tot")) %>%
  pivot_longer(cols=c(totcost, totercost, toteecost))
...
tab <- tabdata %>%
  ...
  tab_header(
      title = paste0("Retiree health plan participants and premium costs in ", j90data_year),
      subtitle = "Age 65+ and Under 65 retirees"
    ) %>%
  cols_label(
    name="",
    mean=html("Mean across all<br>districts with<br>OPEB benefits"),
...
fnbase <- paste0("table_j90_sfusd_vs_means_allopeb_cranesort", j90data_year)
gtsave(tab, here::here("results", paste0(fnbase, ".png")))  # , zoom = 1.5, expand = 10)
# gtsave(tab, here::here("results", "j90_sfusd_vs_means_allopeb_cranesort.png"))  # , zoom = 1.5, expand = 10)
